...
class DDoSAttackGUI:
    def __init__(self, master):
        ...
        self.log_text.pack()
        self.log_text.insert(tk.END, "Attack Log:\n")
    ...
    def send_request(self, url, headers, proxy):
        try:
            ...
            if response.status_code == 200:
                ...
                self.attack_log += f"Request sent to {url} successfully.\n"
                self.log_text.delete(1.0, tk.END)
                self.log_text.insert(tk.END, self.attack_log)
            else:
                print(f"Failed to send request to {url}. Status code: {response.status_code}")
                self.attack_log += f"Failed to send request to {url}. Status code: {response.status_code}\n"
                self.log_text.delete(1.0, tk.END)
                self.log_text.insert(tk.END, self.attack_log)
        except Exception as e:
            print(f"Error sending request to {url}: {e}")
            self.attack_log += f"Error sending request to {url}: {e}\n"
            self.log_text.delete(1.0, tk.END)
            self.log_text.insert(tk.END, self.attack_log)
